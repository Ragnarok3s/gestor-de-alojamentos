...
module.exports = function registerBackoffice(app, context) {
  ...
  function parseRateRuleFormInput(form, existingRule = null) {
...
app.post('/admin/rates/rules/save', requireLogin, requirePermission('rates.manage'), (req, res) => {
  try {
    ...
    const redirectUrl = `/admin/rates/rules?error=${encodeURIComponent(message)}${req.body.rule_id ? `&edit=${encodeURIComponent(req.body.rule_id)}` : ''}`;
